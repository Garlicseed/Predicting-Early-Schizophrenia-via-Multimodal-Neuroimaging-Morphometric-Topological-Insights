...
% Version 2:
%
% This script was orignally obtained from Github (https://github.com/QunjunLIANG/MDD_spatiotemporal_discoordinate/tree/master)
% which was created by Liang Qunjun.
%
% 
...
wkDir          = 'H:\Schizophrenia\derivative1\timeseries_for_schaefer400_independent\';
outdir         = 'H:\Schizophrenia\time_delay\time_lag_estimation';    % set directory for saving out images here

lag_lim        = 4;    % lag limit (in seconds)
lags           = -3:3;    % range of TR shifts; max(lags) = round(lag_lim/tr + 1)

% Specify data parameters
sbjNamePath    = 'H:\Schizophrenia\derivative1\';
subject_list = readtable([sbjNamePath 'participants.tsv'], 'FileType', 'text', 'Delimiter', '\t', 'TreatAsEmpty', 'NA', 'ReadVariableNames', true);
subjects       = subject_list.subID;

% The Yeo 7 network identification for each ROIs
Yeo_7_path     = 'H:\Schizophrenia\time_delay\scripts\Yeo_net_identity_7.csv';
...
for s = useSubject
    ...
    if strcmp(subj, 'sub-061')
    % ����� sub-061���ض� format �� BOLD ������
        format = format(1:size(BOLD, 1));
    end
    ...
    csvwrite([outdir '\' subj '_timeDelay.csv'], subj_lags);
    csvwrite([outdir '\' subj '_peakCorrelation.csv'], subj_peak);
    csvwrite([outdir '\' subj '_zeroLag.csv'], subj_ZL);
    ...
    csvwrite([outdir '\' subj '_projection_map_weighted.csv'], subj_lags_proj);
    csvwrite([outdir '\' subj '_projection_map_unweighted.csv'], subj_lags_proj_unweighted);
    ...
    csvwrite([outdir '\' subj '_probaFlow_projectionMap.csv'], subj_prob_flow_projMap);
    ...
    csvwrite([outdir '\' subj '_probaFlow.csv'], subj_prob_flow);
    csvwrite([outdir '\' subj '_probaFlow_projectionMap_normalization.csv'], subj_prob_flow_projMap);
...
% save the plots
saveas(1, [outdir '\' outFile_prefix '_group_lags_plot.png'])
saveas(2, [outdir '\' outFile_prefix '_group_peak_plot.png'])
saveas(3, [outdir '\' outFile_prefix '_group_zeroLag_plot.png'])
saveas(4, [outdir '\' outFile_prefix '_group_probabilityFLow_plot.png'])
...
csvwrite([outdir '\' outFile_prefix '_group_probability_flow_projection_map.csv'], grp_prob_flow_proj);
...
csvwrite([outdir '\' outFile_prefix '_group_mean_lags.csv'], grp_lags_mean);
csvwrite([outdir '\' outFile_prefix '_group_mean_peak.csv'], grp_peak_mean);
csvwrite([outdir '\' outFile_prefix '_group_mean_zeroLag.csv'], grp_ZL_mean);
csvwrite([outdir '\' outFile_prefix '_group_probability_flow.csv'], grp_prob_flow_mean);

% export group-level projection maps
csvwrite([outdir '\' outFile_prefix '_group_projection_map_unweighted.csv'], grp_lags_proj_unweighted);
csvwrite([outdir '\' outFile_prefix '_group_projection_map_weighted.csv'], grp_lags_proj);
csvwrite([outdir '\' outFile_prefix '_group_probability_flow_projection_map_normalization.csv'], grp_prob_flow_proj);
